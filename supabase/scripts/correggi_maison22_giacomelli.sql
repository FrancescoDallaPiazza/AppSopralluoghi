...
--    nome e la stessa sede ("Verona"). Nell'anteprima dell'import il gruppo
--    MAISON 22 va ESCLUSO, non abbinato a mano: abbinato a cc7d7e47 ricreerebbe
--    li' le 4 persone spostate. Si chiude solo correggendo la sede nel gestionale.
--
...
-- riga sia quella attesa e che niente punti a quello che si toglie (chiavi
-- esterne lette dal catalogo al momento del lancio, e le azioni che portano la
-- chiave nel testo). Ogni scrittura controlla quante righe tocca. Dopo, si
-- verificano i conteggi rispetto a prima. Se un controllo fallisce, errore e
-- niente scritto. Rilanciarlo e' innocuo: i controlli iniziali non trovano piu'
-- le righe attese e annullano.
--
-- ASCII-only.
...
begin
  ...
  raise notice 'Controllo superato: MAISON 22 divisa, AZIENDA AGRICOLA GIACOMELLI tolta, 2 schede tolte da Aprili.';
end $$;
